...
from app.utils.form_validation import name_validator, password_validator
...
class RolesClass(Resource):
    ...
    def put(self): #edit new role
        ...
        if not name_validator(edited_role):
            app.logger.info("Invalid role name")
            return jsonify(status=400, message="Invalid role name")
        check_super_admin = User.query.filter_by(id=user_id).first()
        if not check_super_admin:
            app.logger.info("user not found")
            return jsonify(status=400, message="user not found")
    ...
    def post(self): #add new role
        ...
        if not name_validator(new_role):
            app.logger.info("Invalid role name")
            return jsonify(status=400, message="Invalid role name")
        check_super_admin=User.query.filter_by(id=user_id).first()
        if not check_super_admin:
