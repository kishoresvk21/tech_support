from app.models_package.models import Technologies,User,Queries,Comments,LikesDislikes, SavedQueries
import ast
...
def query_serializer(query_obj, logged_in_user_id):
    saved_or_not = SavedQueries.query.filter(and_(SavedQueries.u_id == logged_in_user_id,
                                                            SavedQueries.q_id == query_obj.id)).first()
    print(saved_or_not)
    ...
    if saved_or_not:
        dt['user_saved_status'] = True
    else:
        dt['user_saved_status'] = False
...
def support_ticket_serializer(support_ticket_obj):
    dt = {
        'support_ticket_id':support_ticket_obj.id,
        'user_id':support_ticket_obj.u_id,
        'name':(User.query.filter_by(id=support_ticket_obj.u_id).first()).name,
        'title':support_ticket_obj.title,
        'description':support_ticket_obj.description,
        'updated_at':support_ticket_obj.updated_at,
        'status':support_ticket_obj.status,
        'userdelete':support_ticket_obj.userdelete
    }
    return dt
